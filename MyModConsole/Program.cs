    ...
    class Program
    {
        ...
        class MainClass
        {
            ...
            private static Game ConfigureGameForTesting()
            {
                ...
                game = new Game(new string[] { "BaronBlade", "Bunker", "Cauldron.Cricket", "TheWraith", "PikeIndustrialComplex" }, advanced, promos, isChallenge: challenge);
            ...
            private static Game ConfigureGameInteractively()
            {
                ...
                Dictionary<string, DeckDefinition> modHeroData = LoadAllModHeroesOrVillains(DeckDefinition.DeckKind.Hero);
                Dictionary<string, DeckDefinition> modVillainData = LoadAllModHeroesOrVillains(DeckDefinition.DeckKind.Villain);
                Dictionary<string, DeckDefinition> modVillainTeamData = LoadAllModHeroesOrVillains(DeckDefinition.DeckKind.VillainTeam);
                Dictionary<string, DeckDefinition> modEnvironmentData = LoadAllModEnvironments();
                ...
                if (input != null && input.ToLower().StartsWith("y"))
                {
                    ...
                    while (!decided)
                    {
                        ...
                        if (gameMode == GameMode.Classic)
                        {
                            ...
                            string villainName = villainDeck.Name;
                ...
                else
                {
                    ...
                    if (fullVillainList.Count > 1)
                    {
                        while (villain == null && villainTeam.Count < 3)
                        {
                            ...
                            if (index > 0 && index <= fullVillainList.Count)
                            {
                                ...
                                List<string> promoDefsString = new List<string>();
                                List<CardDefinition> promoDefinitions = new List<CardDefinition>();
                                if (availableVillains.Contains(villain))
                                {
                                    villainDeck = DeckDefinitionCache.GetDeckDefinition(villain);
                                    promoDefinitions = villainDeck.PromoCardDefinitions.Where(d => DeckDefinition.AvailablePromos.Contains(d.PromoIdentifier)).ToList();
                                }
                                else
                                {
                                    villainDeck = modVillainData[villain];
                                ...
                                promoDefsString = promoDefinitions.Select(cd => cd.QualifiedPromoIdentifierOrIdentifier).ToList();
                                string name = villainDeck.Name;

                                if (promoDefsString.Count() > 0)
                                {
                                    // Ask which version
                                    ...
                                    if (input != null)
                                    {
                                        if (input == "r")
                                        {
                                            index = rng.Next(promoDefsString.Count()) + 1;
                                        }
                                        else
                                        {
                                            index = input.ToInt(-1);
                    ...
                    if (fullEnvironmentList.Count > 1 && villain != "OblivAeon")
                    {
                    ...
                    else if (gameMode == GameMode.OblivAeon)
                    {
                        ...
                        while (environments.Count() < 5)
                        {
                            ...
                            if (index > 0 && index <= fullEnvironmentList.Count)
                            {
                                ...
                                string name = currentEnvironment.Name;
                    ...
                    while (heroes.Count < 5)
                    {
                        ...
                        else if (index > 0 && index <= fullHeroList.Count)
                        {
                            ...
                            var promoDefsString = new List<string>();
                            if (availableHeroes.Contains(identifier))
                            {
                            ...
                            promoDefsString = promoDefinitions.Select(cd => cd.PromoIdentifierOrIdentifier).ToList();
                            ...
                            if (promoDefsString.Count() > 0)
                            {
                                ...
                                do
                                {
                                    ...
                                    if (input != null)
                                    {
                                        if (input == "r")
                                        {
                                            index = rng.Next(promoDefsString.Count() + 2);
                                        }
                                        else
                                        {
                                            index = input.ToInt(-1);
            ...
            private static Dictionary<string, DeckDefinition> LoadAllModEnvironments()
            {
                Dictionary<string, DeckDefinition> modEnvironments = new Dictionary<string, DeckDefinition>();
                foreach (string space in NamespaceList)
                {
                    ...
                    foreach (var res in assembly.GetManifestResourceNames())
                    {
                        ...
                        if (kind != null && kind != DeckDefinition.DeckKind.Environment.ToString())
                            continue;

                        string qualifiedIdentifier = $"{space}.{name}";
                        jsonObject.Add("identifier", new JSONValue(qualifiedIdentifier));
                        modEnvironments.Add(qualifiedIdentifier, new DeckDefinition(jsonObject, space));
                    }
                }
                return modEnvironments;
            }

            private static Dictionary<string, DeckDefinition> LoadAllModHeroesOrVillains(DeckDefinition.DeckKind deckKind)
            {
                Dictionary<string, DeckDefinition> heroesDictionary = new Dictionary<string, DeckDefinition>();
                foreach (string space in NamespaceList)
                {
                    Assembly assembly = ModHelper.GetAssemblyForNamespace(space);
                    if (assembly == null) continue;
                    ...
                    foreach (var res in assembly.GetManifestResourceNames())
                    {
                        ...
                        //add exception for phase - otherwise errors will occur
                        qualifiedIdentifier = qualifiedIdentifier == "Cauldron.Phase" ? "Cauldron.PhaseVillain" : qualifiedIdentifier;
                        jsonObject.Add("identifier", new JSONValue(qualifiedIdentifier));
                        heroesDictionary.Add(qualifiedIdentifier, new DeckDefinition(jsonObject, space));
                    }
                }
                        ...
                        if (decision is YesNoDecision)
                        {
                            ...
                            switch (yesNo.SelectionType)
                            {
                                ...
                                case SelectionType.Custom:
                                    {
                                        CustomDecisionText customDecisionText = null;
                                        if (decision.CardSource != null && decision.CardSource.CardController != null)
                                        {
                                            customDecisionText = decision.CardSource.CardController.GetCustomDecisionText(decision);

                                        }
                                        if (customDecisionText != null)
                                        {
                                            question = address + customDecisionText.Strings[CustomDecisionText.Key.Question];
                                        }
                                        break;
                                    }
            ...
            private static bool? GetBoolean(JSONObject jsonObject, string key)
            {
